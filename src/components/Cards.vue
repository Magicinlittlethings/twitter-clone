<template lang="html">
  <div @mouseover="hover = true" @mouseleave="hover = false" class="relative">
      ...
      <div v-if="hover" class="absolute bg-gray-800 opacity-50 inset-0 w-full h-full rounded-lg"></div>
      <div v-if="hover" class="absolute top-0 w-full h-full flex flex-col justify-between p-3">
          ...
          <div class="flex items-center w-full">
              <button type="button" name="button" class="ml-auto mr-2 bg-white rounded-full w-8 h-8 flex items-center justify-center text-base  text-dark opacity-75 hover:opacity-100">
                  <span class="material-icons">ios_share</span>
              </button>
               <button type="button" name="button" class="bg-white rounded-full w-8 h-8 flex items-center justify-center text-base text-dark opacity-75 hover:opacity-100">
