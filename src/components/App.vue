 ...
 <div id="app" class="flex container h-screen w-full" >
   ...
   <div class="lg:w-1/5 border-r border-lighter px-2 lg:px-6 py-2 flex flex-col justify-between">
    <div>
     ...
     <div>
       <button v-for="tab in tabs" :key="tab" @click="id = tab.id" :class="`hover:text-blue flex items-center py-2 px-4 hover:bg-lightblue rounded-full mr-auto mb-3 ${ id === tab.id ? 'text-blue' : ''}`">
         <i :class="`${tab.icon} text-2xl mr-4 text-left`"></i>
         <p class="text-lg font-semibold text-left hidden lg:block"> {{tab.title}}</p>
       </button>
     </div>
     <button class="text-white bg-blue rounded-full font-semibold w-12 h-12 lg:h-auto lg:w-full p-3 hover:bg-darkblue mb-36">
      <p class="hidden lg:block">  Tweet </p>
      <i class="fas fa-plus  lg:hidden" ></i>
     </button>
    </div>
    <div class="lg:w-full relative">
      <button @click="dropdown = true" class="flex items-center w-full hover:bg-lightblue rounded-full p-2">
        ...
        <i class=" hiden lg:block fas fa-angle-down ml-auto text-lg"></i>
      </button>
   ...
   <div class="w-1/2 h-full overflow-y-scroll">
     ...
     <div class="px-5 py-3 border-b-8 border-lighter flex">
       ...
       <form v-on:submit.prevent = "addNewTweet" class="w-full px-4 relative">
         ...
         <button type="submit" class="h-10 px-4 text-white font-semibold bg-blue hover:bg-darkblue rounded-full absolute bottom-0 right-0">
           Tweet
         </button>
       </form>
     </div>
   ...
   <!-- trending -->
   <div class="w-1/3 hidden md:block h-full border-l  border-lighter py-2 px-6  overflow-y-scroll relative">
    <input class="pl-12 border rounded-full w-full p-2 bg-lighter text-sm mb-4" placeholder="Search Twitter" />
    ...
    <div class="w-full rounded-lg bg-lightest">
      ...
      <button v-for="trend in trends" :key="trend" class="w-full flex justify-between hover:bg-lighter p-3 border-t border-lighter">
        <div>
 ...
 data(){
     return{
       tabs:[
         ...
         {icon:'fas fa-user', title:'User', id:'user'},
    ...
    trends:[
         ...
         {top:'Trending', title:'When Beyonce', bottom:'20.45K Tweets'},
       ...
       following:[
         {src:'elon.jpg', name:'Elon Musk', handle:'@teslaBoy', time:'20 min', tweet: 'should I just quarantine on mars??', comments:'1000', retweets: '550', like:'1,000,003'},
         {src:'monk.jpg', name:'Adrian Monk', handle:'@detective:)', time:'55 min', tweet: 'should me and the rock do another sub-par movie together???', comments:'2000', retweets: '50', like:'200,003'},
         {src:'kevin.jpg', name:'Kevin Hart', handle:'iniRock', time:'1.4hr', tweet: 'Just made a flame thrower, thinking of selling them!!!', comments:'1000', retweets: '550', like:'1.4 hr',},
          {src:'elon.jpg', name:'Elon Musk', handle:'@teslaBoy', time:'20 min', tweet: 'should I just quarantine on mars??', comments:'1000', retweets: '550', like:'1,000,003'},
         {src:'monk.jpg', name:'Adrian Monk', handle:'@detective:)', time:'55 min', tweet: 'should me and the rock do another sub-par movie together???', comments:'2000', retweets: '50', like:'200,003'},
         {src:'kevin.jpg', name:'Kevin Hart', handle:'iniRock', time:'1.4hr', tweet: 'Just made a flame thrower, thinking of selling them!!!', comments:'1000', retweets: '550', like:'1.4 hr',},
          {src:'elon.jpg', name:'Elon Musk', handle:'@teslaBoy', time:'20 min', tweet: 'should I just quarantine on mars??', comments:'1000', retweets: '550', like:'1,000,003'},
         ...
         tweets:[
         {content: 'It is so nice ouutside!'},
